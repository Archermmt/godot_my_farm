...
func test_map_manager_requires_explicit_hosts() -> void:
	...
	assert_equal(router.setup(), ERR_INVALID_PARAMETER)
	router.free()


func test_item_manager_owns_global_unique_id_count() -> void:
	var manager := ItemManagerService.new()
	var first := manager.get_unique_id(&"tree")
	var second := manager.get_unique_id(&"pickup_wood")
	assert_true(first.begins_with("tree_"))
	assert_true(second.begins_with("pickup_wood_"))
	manager.register_unique_id(&"rock_40")
	assert_true(manager.get_unique_id(&"tree").begins_with("tree_"))
...
func test_unconfigured_save_and_audio_operations_return_errors() -> void:
	...
	assert_equal(game_manager.save_game(0), ERR_UNAVAILABLE)
	assert_equal(audio_manager.stop_audio(&"ui_confirm"), ERR_DOES_NOT_EXIST)
	assert_equal(effect_manager.setup(), OK)
	game_manager.free()
	audio_manager.free()
	effect_manager.free()

...
func test_item_state_manages_enum_flags_without_duplicates() -> void:
	var state := ItemState.new()
	assert_true(state.has_flag(ItemMeta.ItemFlag.AVAILABLE))
	state.add_flag(ItemMeta.ItemFlag.HURT)
	state.add_flag(ItemMeta.ItemFlag.HURT)
	assert_true(state.has_flag(ItemMeta.ItemFlag.HURT))
	assert_equal(state.flags.count(ItemMeta.ItemFlag.HURT), 1)
	state.remove_flag(ItemMeta.ItemFlag.HURT)
	assert_true(not state.has_flag(ItemMeta.ItemFlag.HURT))
...
func test_map_rejects_invalid_nested_state_and_duplicate_ids() -> void:
	assert_true(MapState.from_dict({
		...
		"items": [{"state_type": "plant", "meta_id": "", "unique_id": "crop_0_0", "cell": {"x": 0, "y": 0}, "health": 1, "flags": [], "last_growth_day": 0}],
...
func test_base_item_state_round_trip_preserves_base_type() -> void:
	...
	assert_equal(restored.unique_id, state.unique_id)
	assert_equal(restored.flags, [ItemMeta.ItemFlag.AVAILABLE])

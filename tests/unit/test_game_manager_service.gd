...
func test_cross_container_exchange_enforces_types_and_merges_atomically() -> void:
	...
	assert_equal(game_manager.player.exchange_container_slots(&"inventory", 1, &"itembar", 3, catalog_service.get_item(&"material_wood"), null), OK)
	assert_equal(game_manager.player.itembar.get_slot(3).item_id, &"material_wood")
	assert_equal(game_manager.player.exchange_container_slots(&"itembar", 3, &"toolbar", 0, catalog_service.get_item(&"material_wood"), catalog_service.get_item(&"tool_hoe")), ERR_UNAVAILABLE)
	game_manager.free()
	catalog_service.free()

...
func test_backpack_add_fills_stack_then_empty_slot() -> void:
	...
	assert_equal(backpack.add_item_partial(&"inventory", &"wood", 7, 5), 7)
	assert_equal(backpack.get_slot(&"inventory", 0).amount, 5)
	assert_equal(backpack.get_slot(&"inventory", 1).amount, 2)
	assert_equal(backpack.count_item(&"inventory", &"wood"), 7)


func test_backpack_full_add_and_insufficient_remove_are_atomic() -> void:
	var backpack := BackpackState.new(2, 0, 0)
	assert_equal(backpack.add_item_partial(&"inventory", &"wood", 10, 5), 10)
	var before := backpack.to_dict()
	assert_equal(backpack.add_item_partial(&"inventory", &"wood", 1, 5), 0)
	assert_equal(backpack.to_dict(), before)
	assert_true(not backpack.remove_item(&"inventory", &"wood", 11))
	assert_equal(backpack.to_dict(), before)


func test_backpack_switch_and_selection() -> void:
	var backpack := BackpackState.new(2, 2, 0)
	assert_equal(backpack.add_item_partial(&"toolbar", &"hoe", 1, 1), 1)
	assert_equal(backpack.add_item_partial(&"inventory", &"parsnip_seed", 8, 99), 8)
...
func test_backpack_round_trip_preserves_named_slots() -> void:
	...
	assert_equal(backpack.add_item_partial(&"itembar", &"parsnip_seed", 12, 99), 12)
...
func test_removing_a_depleted_stack_compacts_itembar_and_inventory_but_not_toolbar() -> void:
	...
	assert_equal(backpack.add_item_partial(&"itembar", &"parsnip_seed", 1, 1), 1)
	assert_equal(backpack.add_item_partial(&"itembar", &"potato_seed", 1, 1), 1)
	assert_equal(backpack.add_item_partial(&"inventory", &"wood", 1, 1), 1)
	assert_equal(backpack.add_item_partial(&"inventory", &"stone", 1, 1), 1)
	assert_equal(backpack.add_item_partial(&"toolbar", &"hoe", 1, 1), 1)
	assert_equal(backpack.add_item_partial(&"toolbar", &"axe", 1, 1), 1)
	...
	assert_true(backpack.remove_item(&"inventory", &"wood", 1))
	assert_equal(backpack.get_slot(&"inventory", 0).item_id, &"stone")

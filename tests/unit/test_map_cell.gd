...
func test_occupancy_prevents_tilling() -> void:
	...
	assert_equal(cell.add_entity_id(&"rock_001"), OK)
	assert_true(cell.has_occupant())
	assert_true(not cell.can_till())


func test_map_cell_rejects_invalid_or_duplicate_entity_id() -> void:
	var cell := MapCell.new(Vector2i(3, 2))
	assert_equal(cell.add_entity_id(&""), ERR_INVALID_PARAMETER)
	assert_equal(cell.add_entity_id(&"rock_001"), OK)
	assert_equal(cell.add_entity_id(&"rock_001"), ERR_ALREADY_EXISTS)

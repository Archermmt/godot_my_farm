...
func test_base_map_restores_and_operates_on_state_dtos() -> void:
	...
	assert_true(farm.items[&"crop_5_9"] is PlantItem)
	assert_true(farm.items[&"crop_5_9"].state == persisted_item)
	assert_true((farm.items[&"crop_5_9"] as PlantItem).plant_state == persisted_item)
	assert_true((farm.items[&"crop_5_9"] as PlantItem).plant_meta is PlantMeta)
...
func test_each_map_owns_a_distinct_tilemap_hierarchy() -> void:
	...
	assert_true(farm.item_host(ItemMeta.WorldType.PLANT) == farm.get_node("MapItems/Plants"))
	assert_true(field.item_host(ItemMeta.WorldType.PLANT) == field.get_node("MapItems/Plants"))
	assert_true(field.item_host(ItemMeta.WorldType.HARVESTABLE) == field.get_node("MapItems/Harvestables"))
	...
	plant_state.cell = Vector2i(5, 9)
	assert_equal(farm.add_item_state(plant_state), OK)
	var plant := farm.get_item(plant_state.instance_id)
	assert_true(plant.get_parent() == farm.get_node("MapItems/Plants"))
	assert_true(plant is PlantItem)
	assert_true((plant as PlantItem).plant_state == plant_state)
	assert_equal(plant.item_id(), plant_state.instance_id)
	assert_equal(farm.item_count(ItemMeta.WorldType.PLANT), 1)
...
func test_base_map_rejects_meta_state_mismatch() -> void:
	...
	invalid_crop.cell = Vector2i(5, 9)
	assert_equal(field.add_item_state(invalid_crop), ERR_INVALID_PARAMETER)
	var harvestable := HarvestableState.new()
	harvestable.instance_id = &"tree_runtime"
	harvestable.meta_id = &"tree"
	harvestable.cell = Vector2i(5, 9)
	assert_equal(field.add_item_state(harvestable), OK)
	var runtime_item := field.get_item(harvestable.instance_id)
	assert_true(runtime_item is HarvestableItem)
	assert_true((runtime_item as HarvestableItem).harvestable_state == harvestable)
	assert_true((runtime_item as HarvestableItem).harvestable_meta is HarvestableMeta)
	field.free()

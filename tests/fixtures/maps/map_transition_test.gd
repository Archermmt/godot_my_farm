...
func _run() -> void:
	...
	if (
		...
		_fail(
			(
				"new game must start outside the farm house | pos=%s expected=%s roof=%s zoom=%s"
				% [
	...
	if (
		not house.roof_layer.visible
		or house.roof_layer.modulate.a >= 1.0
		or player.camera.zoom != player.indoor_camera_zoom
	):
	...
	await get_tree().physics_frame
	await get_tree().create_timer(player.camera_zoom_duration + 0.05).timeout
	if not house.roof_layer.visible or house.roof_layer.modulate.a < 1.0 or player.camera.zoom != Vector2.ONE:
	...
	if duplicate_result != OK or busy_result != ERR_BUSY:
		_fail(
			(
				"duplicate request contract failed | first=%s second=%s"
				% [error_string(duplicate_result), error_string(busy_result)]
			)
		)
		return
	...
	if router.current_map_id() != &"beach" or player_count != 1 or router.is_transitioning():
		_fail(
			(
				"farm -> beach contract failed | map=%s players=%d transitioning=%s"
				% [router.current_map_id(), player_count, router.is_transitioning()]
			)
		)
		return
	...
	if (
		...
		_fail(
			(
				"day transition did not finish at the indoor wake point | map=%s day=%d time=%02d:%02d pos=%s wake=%s roof=%s overlay=%s transitioning=%s"
				% [

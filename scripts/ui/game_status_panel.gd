...
func _process(_delta: float) -> void:
	# PlayerState is a pure DTO, so polling keeps the panel current for direct state mutations.
	_refresh()
...
func _refresh() -> void:
	...
	var snapshot := "%d/%d/%d/%d/%d/%d|%d/%d/%d/%d/%d|%d/%s/%d" % [
	...
	calendar_label.text = "D%02d  %02d:%02d  %s Y%d" % [
		calendar.day, calendar.hour, calendar.minute, String(calendar.season()).to_upper(), calendar.year,
	]

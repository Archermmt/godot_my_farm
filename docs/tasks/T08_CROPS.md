...
实现种子行动、CropEntity/CropState、生命阶段投影和基于前一日浇水的成长，使“翻地 -> 播种 -> 浇水 -> 次日成长”成为可验证闭环。
...
3. 每个 crop 有稳定 instance ID，FarmCellState 保存 CropState 并占用格子；节点可由状态重建。
...
7. CropRoot y-sort 和格中心对齐；阶段变化不能改变占用或出现一帧空白。
...
通过真实输入翻地、选择种子、多格播种、浇水；使用受控测试入口推进一天，截图至少三个阶段。检查种子数量、CropState 和 CropRoot 节点数；map 往返后仍一致，日志清洁。

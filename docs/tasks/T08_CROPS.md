...
实现种子行动、PlantMeta/PlantState/PlantItem、生命阶段投影和基于前一日浇水的成长，使“翻地 -> 播种 -> 浇水 -> 次日成长”成为可验证闭环。
...
2. 多格播种目标数不能超过 Itembar active stack 数；预览已截断，commit 后数量和实际 crop 数严格一致。
3. 每个地图 Item 有稳定 instance ID；MapState.items 以 ItemState 基类持有只含可变数据和 meta_id 的 PlantState，CellState.item_ids 保存引用；BaseMap 只接受 PlantMeta/PlantState 组合并重建 PlantItem。
...
- PlantItem free 后从 state 重建，节点数和占用不重复。

...
- 地图往返 20 次实体 ID 集合和数量不增长。
- 采集一个、留下一个 Pickup、改变一个 Crop/Tree 后往返，全部状态准确恢复。
- 满地图/无合法候选时有限退出并给出 summary，不死循环。
- ScenePort/spawn 周围安全区无生成物。

## godot-ai 验收

截取 farm/field 首次生成画面，采集/留下掉落后往返，再截恢复画面。运行时对比 entity IDs/count；进行多次快速往返并检查节点/信号/性能 monitor 不增长，日志无重复 ID/占用冲突。

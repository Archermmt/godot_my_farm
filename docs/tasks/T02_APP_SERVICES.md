...
实现架构规定的 Autoload 骨架、DataCatalog 启动校验、GameManager 新游戏初始化和 Main 注入链路，使应用服务可以在没有具体农场内容时可靠启动与重置。
...
- `scripts/autoload/`：event_bus、data_catalog、game_manager、scene_manager、audio_manager。
- `scripts/app/main.gd`：注册 MapHost/ActorHost/UILayer 等场景依赖。
- `project.godot`：仅按架构顺序注册 5 个游戏 Autoload。
...
7. Autoload 初始化顺序严格为 EventBus -> DataCatalog -> GameManager -> SceneManager -> AudioManager。
...
1. autoload_manage list 读回 5 个游戏服务及正确顺序。

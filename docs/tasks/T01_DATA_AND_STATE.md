...
- `scripts/state/`：ItemStack、InventoryState、PlayerState、CalendarState、CropState、FarmCellState、WorldEntityState、NpcState、MapState。
...
5. FarmCellState 使用 `watered_on_day`，CropState 保存 growth_days/health/seed；MapState 可容纳动态实体和 NPC。
...
- 测试 PlayerState 上下限。
- 测试 Farm/Crop/MapState 深度 round-trip，并断言恢复后的 Vector2i/StringName/数值类型。

...
10. 清理代码时扫描所有非 `addons` 的 GDScript：删除无运行时调用的死代码、仅供测试/调试观测且不属于业务 API 的函数，以及与业务逻辑无关的临时辅助代码；测试应改为验证实际业务接口或状态。
...
STATUS 写完整回归表、失败路径、性能数据、run_id 和截图索引；总表 T17 completed。

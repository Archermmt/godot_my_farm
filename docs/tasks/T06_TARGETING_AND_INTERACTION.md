...
- `PlayerInteractionController` 接入唯一 active ItemStack、BaseMap 和 Hands。
...
8. 不在 InteractionController 写锄头/种子/斧头名称判断；通过 ItemMeta.use_kind 分发。

## 自动化验收

- 固定地图中心/边缘和四个朝向的范围顺序正确、结果稳定。
- holding frame 达阈值时 level 只增加一次，release 只提交一次。
- active stack 数量不足时 preview 截断；零数量或无体力给出明确 invalid reason。

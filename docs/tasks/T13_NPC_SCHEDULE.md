...
- `scripts/actors/npc_schedule_controller.gd`、`npc_navigator.gd`、`npc_visual.gd`。
...
7. GameManager.npcs 以 npc_id 保持全局唯一；SceneManager 仅重建 NpcState.map_id 对应当前地图的 NPC Actor。NPC 不使用 ItemState、MapState.items 或 BaseMap 的 Item host；地图往返和 save/load 不重复实例。
8. 路径不可达时 NPC 留在最近安全位置、记录一次 warning，并在下一日程/地图变化时重试，不死循环。

## 自动化验收

- 季节/星期过滤、冲突校验、fallback、跨午夜事件。
- AStarGrid2D 绕过障碍、道路 penalty、生效的对角防切角、不可达终止。
